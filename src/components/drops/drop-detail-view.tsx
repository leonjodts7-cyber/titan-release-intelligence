...
import { getDropMeta, getDropAt } from "@/lib/drop";
  ...
  return (
    <div className="space-y-4 max-w-3xl">
      <header className="intel-card">
        ...
        {!verified && <p className="text-xs text-zinc-400 mt-3">{whyTierLine(release)}</p>}
        <div className="mt-3 flex flex-wrap items-center gap-3">
          <SourceOrigin release={release} />
          {verified && checked && (
            <p className="text-[10px] text-titan-muted">
              {t("dropDetail.lastChecked", { when: checked })}
            </p>
          )}
          {!verified && checked && release.source_name && (
            <p className="text-[10px] text-titan-muted">
      ...
      {verified && (release.price_min != null || release.price_max != null) && (
        <section className="intel-card">
          <h2 className="intel-section-title mb-2">{t("dropDetail.officialPricing")}</h2>
          <p className="text-sm font-mono text-zinc-200">
            {release.price_min != null && release.price_max != null
              ? `${formatEur(release.price_min)} – ${formatEur(release.price_max)}`
              : formatEur(release.price_min ?? release.price_max)}
          </p>
        </section>
      )}
